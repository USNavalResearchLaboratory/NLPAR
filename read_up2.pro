function read_up2, filename, read_interval=read_interval, header=header
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
...
; Date: 5 Dec 2018   
; a file reader for a .up2 from EDAX
; version 1 or 3
...
;                                         relevant header information for the .up2 in a structure.
...
;  version 1 will have dummy values for some of the header info
  ...
  if n_elements(header) ne 0 then begin
    ...
    if version eq 1 then begin
      ...
      pat_width = sz[0]
      pat_height = sz[1]
      N = Round((file_info.size-16ll)/(sz[0]*sz[1]*2ll))
  ...
  file_pos = ulong64(file_pos0)+ulong64(read_interval[0])*ulong64(stride)
  
  ; ready to start reading patterns
  point_lun, lun, file_pos
  
  line = uintarr(sz[0], sz[1])  
